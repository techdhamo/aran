...
@objc public class DeviceStatus: NSObject, Codable {
    ...
    public let screenMirroring: Bool        // explicit mirroring detection
    public let timeSpoofing: Bool           // Device time tampered
    public let locationSpoofing: Bool       // Mock location active
    public let unsecureWifi: Bool           // Open WiFi without encryption
    ...
    public let eventId: String              // Unique per-check event ID
    public let nativeThreatMask: UInt32     // Raw C engine bitmask for diagnostics
    ...
    public var hasThreat: Bool {
        return isJailbroken || fridaDetected || debuggerAttached ||
               ...
               screenMirroring || timeSpoofing || locationSpoofing || unsecureWifi ||
    ...
    public var threatCount: Int {
        ...
        if screenMirroring { count += 1 }
        if timeSpoofing { count += 1 }
        if locationSpoofing { count += 1 }
        if unsecureWifi { count += 1 }
    ...
    public init(
        ...
        screenMirroring: Bool = false,
        timeSpoofing: Bool = false,
        locationSpoofing: Bool = false,
        unsecureWifi: Bool = false,
        ...
        self.screenMirroring = screenMirroring
        self.timeSpoofing = timeSpoofing
        self.locationSpoofing = locationSpoofing
        self.unsecureWifi = unsecureWifi
        ...
        self.appId = appId
        self.nativeThreatMask = nativeThreatMask
