...
internal class AranSigilEngine {
    ...
    func generateCryptogram(nonce: Data) throws -> Data {
        ...
        // Length constraint - max allowed nonce length is 150 bytes
        guard nonce.count <= 150 else {
            throw AranError.invalidArgument("Nonce length exceeds maximum of 150 bytes (got \(nonce.count))")
        }
